...
async function handleSchedule() {
  ...
  const duePullRequests = getDuePullRequests(pullRequests);
...
function getDuePullRequests(pullRequests) {
  ...
  return pullRequests.filter(
      (pullRequest) => new Date(pullRequest.scheduledDate) < new Date(localeString)
  );
}
